...
import { fetchDataKelas, deleteKelas } from 'src/store/apps/kelas/index'
...
const RowOptions = ({ uid }: { uid: any }) => {
  ...
  const handleDelete = async () => {
    try {
      await dispatch(deleteKelas(uid)).unwrap()
      await dispatch(fetchDataKelas({ school_id, status: '', q: value }))
...
const columns: GridColDef[] = [
  ...
  { field: 'class_name', headerName: 'Nama Kelas', flex: 0.175, minWidth: 140 },
