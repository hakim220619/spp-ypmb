...
import TableHeader from 'src/pages/ms/templateMessage/TableHeader'
import { useRouter } from 'next/router'
import toast from 'react-hot-toast'
import { fetchDataTemplateMessage, deleteTemplateMessage } from 'src/store/apps/templateMessage/index'
...
const RowOptions = ({ id }: { id: any }) => {
  ...
  const handleRowEditedClick = () => router.push('/ms/templateMessage/' + id)

  const handleDelete = async () => {
    try {
      await dispatch(deleteTemplateMessage(id)).unwrap()
      await dispatch(fetchDataTemplateMessage({ school_id, q: value }))
...
const PermissionList = () => {
  ...
  const dispatch = useDispatch<AppDispatch>()
  const store = useSelector((state: RootState) => state.TemplateMessage)

  useEffect(() => {
    setLoading(true)
    dispatch(fetchDataTemplateMessage({ school_id, q: value })).finally(() => {
      setLoading(false)
    })
  ...
  return (
    <Grid container spacing={6.5}>
      ...
      <Grid item xs={12}>
        <Card>
          <CardHeader title='Data Template Pesan' />
