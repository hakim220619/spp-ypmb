...
import { ListPaymentReportDate } from 'src/store/apps/laporan/date'
...
interface TabelReportPaymentDateProps {
  school_id: any
  date_first: any
  date_last: any
}

const TabelReportPaymentDate = ({ school_id, date_first, date_last }: TabelReportPaymentDateProps) => {
  const [value, setValue] = useState<string>('')
  const [paginationModel, setPaginationModel] = useState({ page: 0, pageSize: 50 })
  const [loading, setLoading] = useState<boolean>(true)
  const dispatch = useDispatch<AppDispatch>()
  const store = useSelector((state: RootState) => state.ListPaymentReportAdminDate)
  const [openPdfPreview, setOpenPdfPreview] = useState(false)
  const [pdfUrl, setPdfUrl] = useState<string | null>(null)
  const [loadingPdf, setLoadingPdf] = useState<boolean>(false)
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        await dispatch(ListPaymentReportDate({ school_id, date_first, date_last, q: value }))
      } catch (error) {
    ...
    fetchData()
  }, [dispatch, school_id, date_first, date_last, value])
  ...
  const createPdf = async () => {
    ...
    if (store.data && store.data.length > 0) {
      ...
      img.onload = () => {
        ...
        const infoLines = [
          { label: 'Dari Tanggal', value: date_first },

          { label: 'Sampai Tanggal', value: date_last }
        ]
...
export default TabelReportPaymentDate
